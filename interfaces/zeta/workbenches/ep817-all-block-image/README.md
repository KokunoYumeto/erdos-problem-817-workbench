...
```sh
python workbenches/ep817-all-block-image/certificates/verify_all_block_image.py
python workbenches/ep817-all-block-image/certificates/audit_all_block_image.py
```
...
The receiving proof is [Finite arithmetic observations and the all-block capacity return](notes/splitzero-receiving-map.md). Its integer coefficient modules, added congruence kernel and source metric are explicit. The free value-basis observation may be rationalized separately; no ring map from the rationals to a finite field is introduced. The full original Zeta quotient is not identified with a finite carry system.
