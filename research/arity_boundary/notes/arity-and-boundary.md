...
## 1. The literal source and the main outcomes

For q>=2 define

\[
H_q(A)=\left\{\sum_{a\in A}c_a a: c_a\in\{0,\ldots,q-1\}\right\},
\quad \mu_{A,q}(x)=\#\{c:\sum c_a a=x\}.
\]

Values in H_q are counted once. The empty generator set has image {0} and one representation. Fix b>sum(B) and retain

\[
 B_m=\bigcup_{j=0}^{m-1}b^jB,\qquad B_0=\varnothing,
 \quad D_q=H_q(B),
...
The map from generator words to D_q^m is onto, with fibre mass product_j mu_(B,q)(d_j). The next evaluation D_q^m -> L_m is also onto; its further fibres are retained rather than assumed singleton.

The two fixed blocks used throughout are

\[
 B_6=\{1,4,5,17,21,22\},\qquad
...
For every m>=0 the new ternary formulas are
...
## 2. A finite theorem for every distinct-image moment

Let D be any finite set of nonnegative integers containing zero, b>=2, W=max(D), and

\[
 C=\left\lfloor\frac{W}{b-1}\right\rfloor.
\]

During conversion of an actual D-word into its ordinary numerical digits, retain

\[
 d+c=z+b c',\qquad z\in\{0,\ldots,b-1\},\quad c,c'\in\{0,\ldots,C\}.
 \tag{2.1}
\]

The bound is invariant: write W=C(b-1)+r with 0<=r<b-1. Then d+c<=bC+r, so its quotient by b is at most C. Values of d exceeding b and every final carry remain admitted.

For a subset R of this carry interval set

\[
 T_z(R)=\{c':\exists c\in R,\ d\in D,\ d+c=z+bc'\}.
 \tag{2.2}
\]

Starting with R_0={0}, retain every reachable subset, including the empty subset when it occurs. This is a finite state set with at most 2^(C+1) elements. Define

\[
 M_{R,S}=\#\{z:T_z(R)=S\},\qquad f(R)=|R|.
\]

**Theorem 2.1.** The exact number of distinct values is

\[
 N_m(D,b)=|L_m(D;b)|=e_{R_0}^{\mathsf T}M^m f.
 \tag{2.3}
\]
...
**Proof.** An integer represented by a length-m D-word has a unique pair consisting of its m ordinary base-b remainder digits and its integer quotient by b^m. Applying (2.1) along any original representation gives that unique remainder word and final carry. Conversely, every final carry in the determinized subset has at least one original D-word, obtained by lifting its successive transition witnesses. Thus every represented integer is counted exactly once by the final weight |R|, regardless of how many carry paths represent it. This proves (2.3).
...
\[
 V_{m+1}(R)=\coprod_{z=0}^{b-1}\bigl(z+bV_m(T_z(R))\bigr).
 \tag{2.4}
\]

For p>=0, let F_(m,p)(R)=sum_(x in V_m(R)) x^p, interpreting x^0=1 even at x=0. Then

\[
 F_{0,p}(R)=\sum_{c\in R}c^p,
\]
...
For the binary B_6 block in base 97 the full matrix, in states {0}, empty, is

\[
 M=\begin{pmatrix}38&59\\0&97\end{pmatrix},\qquad f=(1,0)^T.
\]
...
## 3. A finite-hole theorem controlling every interval merger

For integers b<w<=2b-2, let H be a nonempty subset of [1,w-b] satisfying

\[
 H\cap((w-b)-H)=\varnothing.
\]

Retain the actual digit set

\[
 D=[0,w]\setminus(H\cup(w-H)),\qquad t=|D|,
\]

and let kappa be the number of its maximal consecutive integer runs.

**Theorem 3.1.** For every m>=0, L_m(D;b) has exactly kappa^m runs, and

\[
 N_m=bN_{m-1}+(t-b)\kappa^{m-1},\qquad N_0=1.
\]
...
**Proof.** Directly from the original holes,

\[
 D\cup(b+D)=[0,w+b]\setminus(H\cup(w+b-H)).
 \tag{3.2}
\]
...
\[
 D+b[L,U]=[bL,bU+w]\setminus
 \bigl((bL+H)\cup(bU+w-H)\bigr).
 \tag{3.3}
\]

It has kappa runs and b(U-L)+t values. Two different runs of the old image have next starting point at least two larger than the preceding endpoint. Their new hulls are separated by at least 2b-w-1>=1 missing integer, so (3.3) for different runs cannot merge. Summing over the runs yields the two recurrences. Solving the geometric sum gives (3.1), including its stated repeated-root case. QED.
...
### 3.1 An independent exact moment recurrence

Write the runs of D as [l_i,u_i], in their actual order. With a=max H there is one central run containing [a+1,w-a-1]. For an incoming run [L,U], every lower output run is anchored at bL, the central output is [bL+l_c,bU+u_c], and every upper output run is anchored at bU. Thus all sums of endpoint powers transform by the binomial formula, with no numerical approximation.

If L_p and U_p are the sums of p-th powers of run endpoints, the exact moment formulas are
...
## 4. The six-generator images at every arity

For B_6 the exact ternary image is

\[
 D_3=[0,140]\setminus\{3,137\}.
\]
...
For q=4, direct original digit enumeration gives D_4=[0,210]. For q>4, add the actual binary image one layer at a time:

\[
 H_{q+1}(B)=H_q(B)+H_2(B).
\]

Once H_q(B) is an interval of length (q-1)70, its translates by 0 and 70 overlap and cover the entire next interval. Hence D_q=[0,(q-1)70] for every q>=4. Lifting an interval digit set whose width is at least b-1 gives another full interval by overlap of consecutive translated intervals. Induction proves
...
For reference, with F_q(z)=sum_(m>=0)|H_q(B_m)|z^m,
...
\[
 F_q(z)=\frac{1+((q-1)70-b)z}{(1-bz)(1-z)}\quad(q\ge4).
 \tag{4.2}
\]

## 5. The ten-generator images: three different merger scales

For B_10 the exact ternary image has t=2103 values in [0,2204]. Its small holes are
...
For every q>=6 and m>=1 there is a simpler complete description. Put

\[
 W_m=(q-1)1102\frac{1651^m-1}{1650},\qquad H_0=\{1,2,5\}.
\]

Then

\[
\boxed{H_q((B_{10})_m)=[0,W_m]\setminus(H_0\cup(W_m-H_0)).}
 \tag{5.4}
\]

**Proof.** Direct enumeration of the original q=4 block gives [0,3306] minus these six reflected holes. All q>=4 local block images have the same small holes: the central intervals of the 0 and 1102 translates overlap at each next arity. The small integers 1,2,5 remain absent because every generator belongs to the semigroup generated by 3 and 4, whose nonnegative gaps are exactly 1,2,5. Reflection of coefficient words gives the high holes.

For q>=6 the local width w=(q-1)1102 is at least 5510. If the preceding lifted image has only the six holes in (5.4), consecutive present integers differ by at most three. The central interval [6,w-6] of each local translate therefore overlaps that of the next translate: w-3*1651>=557>11. These central intervals cover the whole middle region. Before the first translated central interval and after the last, the original low and high holes remain. This proves the induction, including the absence of any other hole. QED.

In particular |H_q|=W_m-5 for m>=1, while |H_q(B_0)|=1. Its series is
...
## 6. A visible nilpotent term in the unchanged carry coordinates

For q=4 and B10/base1651 the complete reachable carry sets, in order, are

\[
 \{0\},\quad\{0,1,2\},\quad\{1,2\},\quad\{1\},\quad\{0,1\}.
\]
...
\[
 \det(XI-M)=(X-1651)(X-3)^2(X-1)^2.
\]

Retain the vectors

\[
 v=(1,0,0,1,0)^T,\qquad w=(0,-411,1,0,1)^T.
\]

Their exact equations are

\[
 (M-3I)v=0,\qquad (M-3I)w=824v.
 \tag{6.2}
\]

Moreover rank(M-3I)=4 and rank((M-3I)^2)=3. The output Hankel matrix (N_(i+j))_(0<=i,j<5) has rank three. Thus the receiving sequence retains the size-two 3-primary block. The inverse resolvent of (6.1) gives (5.1); its double pole supplies the actual -(2m/3)3^m term. Replacing this block by its eigenvalue alone would change the numerical image cardinality.

In the forward system T=M^T, the kernel of the full observation (2.6) has dimension two. It has the displayed basis

\[
 (-1,0,0,1,0)^T,\qquad(0,0,-1,0,1)^T.
\]
...
## 7. Canonical mergers and the exact original quotient metrics

### 7.1 All levels join at the ternary stage

The binary factorization into the original B6 or B10 blocks is exact. Within each block the original triangle relations connect all coordinates, so these are precisely its binary components in the predecessor's canonical bounded-relation factorization. Binary digit uniqueness prevents a relation between different numerical block values.
...
Let Omega be the original q-digit word set of size q^(m|B|). Its local quotient and receiving map are

\[
 \mathbb Q[\Omega]\xrightarrow{q_{\rm loc}}\mathbb Q[D_q^m]
...
\[
\boxed{0\to\ker q_{\rm loc}\to\ker(J_mq_{\rm loc})
...
\[
 \dim\ker J_m=|D_q|^m-|H_q(B_m)|.
 \tag{7.3}
\]

For example, its ternary dimensions are 139^m-(68*97^m-21*3^m)/47 and 2103^m-(1007*1651^m-226*89^m)/781. This is an exponential loss from the independent local-value product. In particular, no lower estimate retaining 139^m, or 2103^m, up to a subexponential divisor can hold for these binary-isolated families.

### 7.2 Weighted least-norm sections

Put omega(d)=mu_(B,q)(d), and Omega(d_0,...,d_(m-1))=product_j omega(d_j). The quotient of the original orthonormal word source gives the local Gram diag(1/Omega(d)). On the final value image define

\[
 \mu_m(x)=\sum_{\sum b^jd_j=x}\Omega(d).
\]

The exact least-norm section is

\[
 s_m(e_x)=\sum_{\sum b^jd_j=x}\frac{\Omega(d)}{\mu_m(x)}e_d,
 \qquad G_m=\operatorname{diag}_x(1/\mu_m(x)).
 \tag{7.4}
\]

The coefficients sum to one. Their squared norm in the original local Gram is 1/mu_m(x). A vector in ker J_m has coefficient sum zero within every final fibre, so it is orthogonal to these section vectors. This proves both the right-inverse and Pythagoras identities, including all original masses.

### 7.3 Exact maximum multiplicity at every length

For any weighted D subset [0,2b-2], its carry coefficients on states c=0,1 are

\[
 W_z(c,c')=\omega(z+bc'-c),\qquad\omega(t)=0\text{ outside }D.
\]

Every row sum is bounded by the largest complete residue-class mass

\[
 \Lambda=\max_{r\bmod b}\sum_{d\equiv r\bmod b}\omega(d).
 \tag{7.5}
\]
...
\[
\boxed{\max_x\mu_{B_m,3}(x)=15^m\text{ or }219^m,}
 \tag{7.6}
\]

and the original quotient-to-unit-value identity has exact norm 15^(m/2) or 219^(m/2).

For comparison, give each distinct local digit weight one instead. Every residue matrix then has entries zero or one. A prescribed final carry receives at most the total mass at the preceding step, so its coefficient is at most 2^(m-1), m>=1. A residue r with r-1,r,b+r-1,b+r all in D makes its matrix all ones and attains that bound by repetition. Such a residue is r=1 for B6 and r=4 for B10. Hence

\[
\boxed{\max_x\#\{d\in D_3^m:\sum b^jd_j=x\}=2^{m-1}.}
 \tag{7.7}
\]
...
## 8. Exact limiting measure of the higher-arity image

The infinite geometry is useful only with its scale and return map explicit. Under Theorem 3.1 additionally assume w<2b-2, and put

\[
 s=\frac{w}{b-1},\quad C_0=\frac{t-\kappa}{b-\kappa},
 \quad r_0=\frac\kappa b,
\]

\[
 K=\left\{\sum_{j\ge1}d_jb^{-j}:d_j\in D\right\},\qquad
 K_m=b^{-m}(L_m(D;b)+[0,s]).
 \tag{8.1}
\]

The scale map is multiplication by b^(-m), with inverse multiplication by b^m. The sets K_m are nested compact supersets with intersection K: the interval [0,s] is invariant under every digit map (d+x)/b, and finite digit approximations have uniformly vanishing tail diameter.

Because 1<s<2, distinct runs of L_m give disjoint intervals in K_m. Their exact total length is
...
\[
\boxed{\operatorname{Leb}(K_m)=C_0+(s-C_0)r_0^m,
 \quad\operatorname{Leb}(K)=C_0.}
 \tag{8.2}
\]

The remaining outer mass is exactly (s-C_0)r_0^m. For the three ternary applications the tuples (C_0,s,s-C_0) are

\[
 (68/47,35/24,13/1128),\quad
 (68/45,35/23,11/1035),\quad
 (1007/781,1102/825,247/5325).
\]

### 8.1 Boundary size, with the notion of dimension specified

The lower and upper box-counting dimensions of the boundary both equal

\[
\boxed{\dim_{\rm box}\partial K=\frac{\log\kappa}{\log b}.}
 \tag{8.3}
\]

To prove the lower bound, both endpoints of every K_m component belong to K: use a terminating allowed word for its lower endpoint, and an all-w tail for its upper endpoint. They are boundary points because the adjacent gap is excluded from K_m. The 2*kappa^m endpoints are separated by at least min(s,2-s)b^(-m).

For the upper bound, write a=max H and theta=(a+1)/(b-1). Choosing the central child of any K_m component [l,u] at every subsequent stage retains the interval [l+theta*b^(-m),u-theta*b^(-m)] in K. Its length is positive: s-2theta>0 follows from a<=w-b and w<2b-2. Thus the boundary inside each component lies in two collars of length theta*b^(-m), each covered by a fixed number of intervals of scale b^(-m). This proves the matching upper bound and, by comparison of intermediate scales, both box dimensions. No Hausdorff-dimension assertion is needed here.

### 8.2 Weak convergence with an evaluated error

Let E_m=b^(-m)(L_m+[0,1]) be the disjoint unit-cell thickening, and retain the unweighted discrete measure

\[
 \eta_m=b^{-m}\sum_{x\in L_m}\delta_{x/b^m}.
\]

One has E_m subset K_m and

\[
 \operatorname{Leb}(E_m\triangle K)
 \le(2s-1-C_0)r_0^m.
\]
...
This proves explicit weak convergence. Division by the actual mass N_m/b^m, whose error is given by (3.1), gives the uniform distinct-image probability limit Leb|K/C_0. The cell smoothing is an explicit comparison map; no total-variation convergence of a discrete probability to a continuous probability is asserted.

## 9. Exact second moments and original variance costs

For the ternary images, reflection gives their exact mean W_m/2, where
...
\[
 \kappa^m,\ b^m,\ (b\kappa)^m,\ b^{2m},\ (\kappa b^2)^m,\ b^{3m}.
\]
...
Let U_m be uniform on the distinct ternary image. The resulting limits are

| Block/base | lim Var(U_m)/b^(2m) | lim Var(U_m)/sum_(a in B_m) a^2 |
|---|---|---|
| B6/97 | 93825139/536649960 | 93825139/71644595 |
| B6/93 | 82897249/434753337 | 82897249/63141789 |
| B10/1651 | 14562026775392709/102939824359874810 | 436860803261781270/275866493425966949 |

The second column also equals the variance of the exact limiting measure in (8.4). The third column retains

\[
 \sum_{a\in B_m}a^2=\left(\sum_{a\in B}a^2\right)
 \frac{b^{2m}-1}{b^2-1}.
\]
...
## 10. A separate finite decision for real-limit progressions

Let D be a finite subset of [0,b-2] containing zero, and define the actual real limit K_b(D) by its digit expansion as in (8.1). The strict span bound max D/(b-1)<1 makes expansions unique: a first differing digit contributes at least b^(-j), while its entire later tail contributes strictly less.

For a k-tuple of digit columns, the integer prefix second differences obey

\[
 c_{j+1}=bc_j+\Delta(d^{(j)}),\qquad c_0=0.
 \tag{10.1}
\]
...
\[
 |c_{j,i}|\le 2\max(D)/(b-1).
\]

Conversely, an infinite digit path with this bounded carry has zero limiting second differences because b^(-m)c_m tends to zero. A persistent flag records whether any column has unequal first two digits; uniqueness of expansions makes it exactly the nonconstant condition.

**Theorem 10.1.** Put C=floor(2max(D)/(b-1)). The finite graph on (c,flag), c in [-C,C]^(k-2), with transitions (10.1), decides whether K_b(D) contains a nonconstant k-AP. Such a progression exists exactly when a reachable flagged state can reach a directed cycle. If it exists, an eventually periodic digit witness exists with preperiod less than the number of graph states and period at most that number.
...
### 10.1 Explicit rational progressions in all the previously used limits

The following are original binary digit columns a,e. Repeating e forever after a gives

\[
 x=\frac1b\left(a+\frac e{b-1}\right).
\]

| Block/base, k | First column a | Repeated column e | Real progression |
|---|---|---|---|
| {1,7,8}/19, 4 | (0,0,1,1) | (0,9,0,9) | (0,1,2,3)/38 |
| B6/97, 5 | (4,5,5,5,6) | (64,0,32,64,0) | (14,15,16,17,18)/291 |
| B6/93, 6 | (0,0,0,0,1,1) | (4,26,48,70,0,22) | (4+22i)/(93*92), 0<=i<6 |
| B10/1651, 6 | (293,293,294,294,294,295) | (548,1099,0,551,1102,3) | (483998+551i)/2724150, 0<=i<6 |
...
\[
 Y_m=b^{m-1}a+\frac{b^{m-1}-1}{b-1}e\in H_2(B_m)^k,
\]
...
## 11. A graph observation retaining the missing boundary

Let Delta:R^k->R^(k-2) be the original second-difference map. Its explicit right inverse is

\[
 R(c)_0=R(c)_1=0,\qquad
 R(c)_i=\sum_{j=0}^{i-2}(i-1-j)c_j\quad(i\ge2).
\]

On the linear space of sequences y_m for which both b^(-m)y_m and Delta y_m converge, retain the pair of observations

\[
 \mathcal O(y)=(\lim b^{-m}y_m,\lim\Delta y_m).
\]

The first component necessarily lies in ker Delta. The pair map is onto ker Delta direct-sum R^(k-2), with explicit section

\[
 (x,c)\longmapsto(y_m=b^m x+R(c))_m.
\]
...
Korsky, S. (2026). *Arithmetic progression-free subset-sum sets* (arXiv:2606.24139v1). https://arxiv.org/abs/2606.24139 . The original problem conventions and general bounds provide context; the inspected abstract is not represented as a new full-paper audit in this continuation.

Frougny, C., & Pelantová, E. (2018). *Two applications of the spectrum of numbers* (arXiv:1512.04234v3; original preprint 2015). https://arxiv.org/abs/1512.04234 . The inspected abstract supplies established context for finite automata and infinite zero representations. The specialized carry theorems above are proved directly; no unread theorem is imported.

KokunoYumeto. (2026). *Reconstruction with changes of support index*. `workbenches/splitzero-tandem/tex/support_diagrams.tex` at Zeta revision `58626a62cd5648e8ae7450fb54d4a4aab2330981`, equations D6–D8; Git blob `d3493f891291ee6e94dbf2c77649f7d85d240df2`. Role: inspected original internal quotient, supported kernel, and additional receiving-kernel interface. https://github.com/KokunoYumeto/zeta-function-research-reader/blob/58626a62cd5648e8ae7450fb54d4a4aab2330981/workbenches/splitzero-tandem/tex/support_diagrams.tex .

The Clankers. (2026, September 16). *Canonical arithmetic factors and integral isolation at every clique rank*. Delivered predecessor `EP817_CANONICAL_ISOLATION_20260916.zip`, retained unchanged in the cumulative package. Role: actual canonical bounded-relation components and their binary graphical instances, with the original mathematical and Lean attributions preserved.
